...
import { getActiveStackFrameId } from '../debugger/debuggerInspector';
...
export class InspectorValuesProvider implements vscode.HoverProvider {
    ...
    constructor(private context: vscode.ExtensionContext) {
        ...
        // Listen to debugger breakpoint pause events to update inline annotations
    ...
    private async updateDecorations(editor: vscode.TextEditor): Promise<void> {
        ...
        // If active debug session, skip static decoration queries to avoid interfering with DAP stepping
    ...
    public async provideHover(
        ...
        // Mode 1: Active Debugger Paused at Breakpoint
        const session = vscode.debug.activeDebugSession;
                ...
                if (wordRange) {
                    ...
                    try {
                        ...
                        if (response && response.result !== undefined) {
                            ...
                            md.isTrusted = true;
                            md.appendMarkdown(`### 🐞 Paused Value: **${wordText}**\n`);
                            md.appendMarkdown(`Value: \`${response.result}\`\n\n`);
                            md.appendMarkdown(`Type: \`${response.type || 'unknown'}\`\n\n`);
                            md.appendMarkdown(`[⚡ Evaluate / Modify Expression](command:antigravity-unity.evaluateExpression "Open Evaluate Expression Window")`);
                            return new vscode.Hover(md, wordRange);
                        }
                    } catch {
                        // Fallthrough to mode 2 if DAP evaluation failed
                    }
                }
            }
        }

        // Mode 2: Static Serialized Field Usages in Unity
        const fields = this.parseSerializableFields(document);
