...
{4}
    ...
    private static List<GeneratedFile> GenerateAllFilesBackground(SyncData syncData)
    {
        ...
        foreach (var assembly in syncData.assemblies)
        {
            ...
            string content = GenerateCsprojContent(assembly, activeNames, syncData.rootNamespace, syncData.applicationContentsPath);
    ...
    private static string GenerateCsprojContent(AssemblySyncData assembly, HashSet<string> generatedAssemblies, string rootNamespace, string applicationContentsPath)
    {
        ...
        AppendCoreUnityReferencesContent(sb, referencedNames, applicationContentsPath);
        sb.AppendLine("  </ItemGroup>");
    ...
    private static void AppendCoreUnityReferencesContent(StringBuilder sb, HashSet<string> existingRefs, string contentsPath)
    {
        var searchPaths = new[]
        {
    ...
    private static List<string> GetAnalyzerPaths(Assembly assembly)
    {
        ...
        // Also search com.antigravity.ide package directories inside PackageCache
        try
        {
            string pcDir = Path.Combine(projectDir, "Library", "PackageCache");
            if (Directory.Exists(pcDir))
            {
                foreach (var dir in Directory.GetDirectories(pcDir, "com.antigravity.ide*"))
                {
